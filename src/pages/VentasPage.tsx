...
import { Search, ShoppingCart, Trash2, Plus, Minus, CreditCard, Loader2, AlertTriangle } from 'lucide-react'
...
export default function VentasPage() {
  ...
  // ── Mapa de stock para consulta O(1) ─────────────────────────
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        // Solo variantes activas con precio de venta definido
        const activas = vars
          .filter(v => v.activo)
          .map(v => ({
            id:     v.id,
  ...
  function agregarItem(v: { id: number; nombre: string; sku: string | null; precio: number }) {
    ...
    setCarrito(prev => {
      ...
      if (existe) {
        // Incrementar si hay stock disponible
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-full">
      ...
      <div className="flex-1 min-w-0 space-y-4">
                ...
                {resultados.map(v => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-tin-pale text-left transition-colors"
                    >
                      <div>
        ...
        <div className="hidden sm:block bg-white rounded-2xl border border-tin/20 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-tin/20 flex items-center gap-2">
            ...
            {carrito.length > 0 && (
              <span className="ml-auto text-xs bg-primary text-white rounded-full px-2 py-0.5 font-semibold">
                  ...
                  <tr key={item.varianteId} className="hover:bg-tin-pale/30 transition-colors">
                    <td className="px-5 py-3">
                      ...
                      <p className="text-xs text-tin">
                        Stock: {item.stockDisponible} {item.unidadAbrev}
                      </p>
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          ...
                          className="w-7 h-7 rounded-lg bg-tin-pale hover:bg-tin/20 flex items-center justify-center transition-colors active:scale-95 duration-100"
                        >
                        ...
                        <button
                          ...
                          className="w-7 h-7 rounded-lg bg-tin-pale hover:bg-tin/20 flex items-center justify-center transition-colors active:scale-95 duration-100 disabled:opacity-40"
                        >
        ...
        {carrito.length > 0 && (
          <div className="sm:hidden bg-white rounded-2xl border border-tin/20 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-tin/10">
              {carrito.map(item => (
                <div key={item.varianteId} className="px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={() => cambiarCantidad(item.varianteId, -1)}
                      className="w-7 h-7 rounded-lg bg-tin-pale flex items-center justify-center active:scale-95 duration-100"
                    >
                      <Minus size={12} className="text-tin-dark" />
                    </button>
                    <span className="w-5 text-center font-semibold text-gray-800 text-sm">{item.cantidad}</span>
                    <button
                      onClick={() => cambiarCantidad(item.varianteId, +1)}
                      disabled={item.cantidad >= item.stockDisponible}
                      className="w-7 h-7 rounded-lg bg-tin-pale flex items-center justify-center active:scale-95 duration-100 disabled:opacity-40"
                    >
                      <Plus size={12} className="text-tin-dark" />
                    </button>
                  </div>
                  ...
                  <button onClick={() => quitarItem(item.varianteId)} className="p-1 text-tin hover:text-red-500 transition-colors">
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
      ...
      <div className="w-full lg:w-80 shrink-0 space-y-4">
        <div className="bg-white rounded-2xl border border-tin/20 shadow-sm p-5 space-y-4">
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold text-sm transition-all duration-150 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          >
